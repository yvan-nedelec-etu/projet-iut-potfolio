# Résumé — Yvan Nedelec

## Qui je suis
Je m'appelle Yvan Nedelec. Je suis étudiant en BUT Science des Données.

## Ce que je fais en ce moment
- Alternance : Data Analyst à la MAIF
- Travail sur Python, SQL, et migration SAS vers Python

## Mes projets académiques (BUT SD)
- Datavisualisation
- Séries chronologiques
- Enquête sur les logements étudiants
- Site web PHP & JavaScript
- Collecte de données web
- Présentation d'un territoire économique et culturel
- Conception et implémentation d'une base de données

## Contact
- Email : yvan.nedelec@etu.univ-poitiers.fr
- LinkedIn : https://fr.linkedin.com/in/yvan-nedelec-40b779214

## Objectifs
Je veux évoluer vers l’analyse de données, le machine learning ou le data engineering.
